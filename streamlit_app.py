...
def generate_edgar_queries_with_local_model(base_prompt: str, max_queries: int = 8) -> tuple[str, str]:
    ...
    try:
        ...
        return derive_edgar_queries_from_prompt(prompt, max_queries=max_queries), f"Local model query generation failed: {exc}"

    return derive_edgar_queries_from_prompt(prompt, max_queries=max_queries), "Used rules-based fallback generation."
...
def render_model_improvement_page():
    ...
    with strategy_tab:
        ...
        if st.button("Generate EDGAR Queries From Prompt", type="primary", use_container_width=True, key="mi_generate_edgar_queries"):
